...
export class LyriaMusicService extends BaseGenAIService {
     ...
     */
    async setWeightedPrompts(session, weightedPrompts) {
        try {
            if (!session ||
                !weightedPrompts ||
            ...
            const fn = session.setWeightedPrompts;
     ...
     */
    async setMusicGenerationConfig(session, config) {
        try {
            if (!session || !config) {
            ...
            const fn = session.setMusicGenerationConfig;
