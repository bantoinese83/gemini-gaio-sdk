...
export class DocumentService extends BaseGenAIService {
     ...
     * @returns The summary text
     */
    async summarizeFromFile(params) {
        try {
     ...
     * @returns The summary text
     */
    async summarizeLargeFromUrl(params) {
        try {
            if (!params.model || !params.url) {
            ...
            const content = [{ text: params.prompt || 'Summarize this document' }];
            ...
            if (err instanceof FileProcessingError || err instanceof ValidationError)
                throw err;
     ...
     * @returns The summary text
     */
    async summarizeLargeFromFile(params) {
        try {
            if (!params.model || !params.filePath) {
     ...
     */
    async summarizeMultiple(params) {
        try {
            if (!params.model ||
                !params.docs ||
            ...
            const { createPartFromUri } = await import('@google/genai');
            const content = [{ text: params.prompt || 'Summarize or analyze these documents' }];
            for (const doc of params.docs) {
                ...
                const file = await FileHelper.uploadFileAndWait(this.genAI, fileInput, doc.displayName || String(doc.file));
                if (file.uri && file.mimeType) {
