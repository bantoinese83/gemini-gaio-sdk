...
import { GenerateImageParams, GenerateImageResult, ContentArray, ModelConfig } from "../types/types";
...
export class ImageService extends BaseGenAIService {
  ...
  async generateImage({ model, contents, config }: { model: string; contents: any[]; config?: ModelConfig }): Promise<GenerateImageResult[]> {
    try {
      if (!model || !contents) {
